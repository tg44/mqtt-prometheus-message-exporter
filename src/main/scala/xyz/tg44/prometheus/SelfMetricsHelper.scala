...
import xyz.tg44.prometheus.exporter.Counter
...
class SelfMetricsHelper(selfMetricsConfig: SelfMetricsConfig) {
  ...
  def uptimeSource: Source[(Line, Boolean), Cancellable] = {
    val meta = MetricMeta(Counter.mType, s"${selfMetricsConfig.prefix}_uptime", Map.empty, "The uptime of the mqtt-exporter", None)
